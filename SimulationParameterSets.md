# Compound Parameter Sets

## Problem Statement

Users working with PK-Sim simulations frequently override simulation parameters that are originally formula-based (BuildingBlockType = Simulation) with constant values. These customized parameter values are valuable and represent validated configurations, but currently they exist only within the simulation context and cannot be:
...
1. **Created** by committing changed simulation parameters back to their respective compounds
...
The system will track uncommitted simulation parameter changes with a visual indicator (orange status) to help users identify parameters that could be committed to Parameter Sets.
...
- **Pavel**: The "Commit" command should be a property of a compound, no? In that case, only the values for this specific compound are committed.
...
- Starts as empty list (no default Parameter Set created automatically)
...
- Dedicated, testable method to determine if a simulation parameter belongs to a compound
...
- **Pavel** should actually never happen
...
- **Pavel**: Did we agree on this? I think it should be a "Compound" action
...
- CommitSimulationParametersCommand (macro command)
- TrackParameterChangeCommand / UntrackParameterChangeCommand
